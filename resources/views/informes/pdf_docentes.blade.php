...
<head>
    <meta charset="utf-8">
    <title>Informe de Repartos</title>
    <style>
...
<body>
    ...
    <table>
        ...
        <tbody>
            @foreach ($datos as $reparto)
                <tr>
                    <td>{{ $reparto->nombre_apellido }}</td>
                    <td>{{ $reparto->dni }}</td>
                    <td>{{ $reparto->institucion }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>

    <p style="margin-top: 20px;"><strong>Total de repartos: {{ $datos->count() }}</strong></p>
</body>
</html>
